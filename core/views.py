...
class AnswerCreateView(CreateView):
  ...
  def form_valid(self, form):
    question = Question.objects.get(id=self.kwargs['pk'])
    if Answer.objects.filter (question=question, user=self.request.user).exists():raise PermissionDenied()
    form.instance.user = self.request.user
  ...
  def form_valid(self, form):
    ...
    try:
      ...
      if not has_voted:
            ...
            prev_votes[0].delete()
      return redirect(reverse('question_detial', args=[form.date["question"]]))
    except:
